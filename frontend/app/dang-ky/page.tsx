...
export function RegisterPageContent({ initialRole = "NGUOI_LAO_DONG", lockRole = false }: { initialRole?: Role; lockRole?: boolean }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorCode === "ACCOUNT_PENDING_VERIFICATION") {
          ...
          const resendResponse = await fetch(`${BACKEND_API_URL}${resendEndpoint}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          const nextOtp =
            resendData.developmentOtp ?? resendData.data?.developmentOtp;
          if (nextOtp) sessionStorage.setItem("developmentOtp", nextOtp);
          const accountType =
      ...
      if (data.developmentOtp ?? data.data?.developmentOtp) {
        sessionStorage.setItem(
          "developmentOtp",
          data.developmentOtp ?? data.data.developmentOtp,
        );
      }
      const accountType =
        role === "NHA_TUYEN_DUNG" ? "&loai=nha-tuyen-dung" : "";
      setTimeout(
        () => router.push(`/xac-thuc-otp?email=${encodeURIComponent(email)}${accountType}`),
        900,
      );
    } catch {
    ...
    <main className="auth-page register-page simple-auth-page">
      ...
      <section className="auth-form-side">
        <div className="auth-form-card register-card">
          ...
          <h2>{role === "NHA_TUYEN_DUNG" ? "Đăng ký nhà tuyển dụng" : "Đăng ký người lao động"}</h2>
          <p className="auth-subtitle">{role === "NHA_TUYEN_DUNG" ? "Tạo hồ sơ doanh nghiệp để đăng tuyển và quản lý ứng viên." : "Tạo tài khoản để xây dựng hồ sơ và ứng tuyển việc làm."}</p>

          {!lockRole && <div className="role-picker">
            <button
          ...
          <form onSubmit={handleSubmit}>
              ...
              <>
                <div className="form-row">
                  ...
                  <label className="form-group">
                    ...
                    <small className="field-hint">Sau khi đăng ký, doanh nghiệp dùng mã số thuế này để đăng nhập.</small>
                    {errorFor("maSoThue")}
                  </label>
                </div>
                <label className="form-group">
            ...
            {role === "NGUOI_LAO_DONG" && <div className="form-row">
              <label className="form-group">
                <span>Họ và tên</span>
          ...
          <p className="auth-switch secondary-switch">
            {role === "NHA_TUYEN_DUNG" ? <>Bạn là người tìm việc? <Link href="/dang-ky">Đăng ký người lao động</Link></> : <>Bạn cần tuyển nhân sự? <Link href="/dang-ky-nha-tuyen-dung">Đăng ký nhà tuyển dụng</Link></>}
          </p>
        </div>
      </section>
    </main>
